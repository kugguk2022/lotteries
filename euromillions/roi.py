...
def build_dataset(draw_df: pd.DataFrame, k: int, bias_cfg: BiasConfig, min_warmup: int):
    ...
    if T <= min_warmup + 1: raise ValueError("Not enough rows for warmup.")
...
def validate_prize_table(prize_table: dict):
    """
    Ensure keys exist for all (km, ks) pairs used in EV and realized payout.
    Warn (and fill with 0.0) when missing.
    """
    missing = []
    for km in range(6):
        for ks in range(3):
            if (km, ks) not in prize_table:
                missing.append((km, ks))
                prize_table[(km, ks)] = 0.0
    if missing:
        print(f"[warn] prize_table missing {len(missing)} tiers (set to 0): {missing[:6]}{'...' if len(missing)>6 else ''}")
    return prize_table

def load_jackpots_csv(path: str):
    """
    Load a CSV that contains a per-draw jackpot column. Expected columns include either:
    - 'jackpot' (numeric), optionally 'date' or draw index
    We simply return a float list in file order.
    """
    df = pd.read_csv(path)
    for c in df.columns:
        if str(c).strip().lower() in ("jackpot","jackpot_eur","jackpot_amount","jackpot_value"):
            vals = pd.to_numeric(df[c], errors="coerce").fillna(0.0).astype(float).tolist()
            return vals, df
    num_cols = [c for c in df.columns if pd.api.types.is_numeric_dtype(df[c])]
    if not num_cols:
        raise SystemExit("[error] No numeric jackpot column found in jackpot CSV.")
    vals = pd.to_numeric(df[num_cols[-1]], errors="coerce").fillna(0.0).astype(float).tolist()
    return vals, df

def inject_dynamic_jackpot(prizes_multi: list, jackpots: list, km=5, ks=2):
    """
    Override the (5,2) prize of each step with the provided jackpots if lengths match (or align from tail).
    """
    if not prizes_multi:
        return prizes_multi
    Lp = len(prizes_multi); Lj = len(jackpots)
    if Lj == 0:
        return prizes_multi
    if Lj != Lp:
        off = max(0, Lj - Lp)
        ...
        off = 0
    for t in range(Lp):
        j = jackpots[off + t] if off + t < Lj else jackpots[-1]
        prizes_multi[t][(km, ks)] = float(j)
    print(f"[info] injected dynamic jackpot for {Lp} steps using {(Lj - off)} jackpot rows starting at offset {off}")
    return prizes_multi
...
def eval_policy(p_agent, truth, p_pair, sig=None, w=(0.7,0.3), tau=1.0,
                ...
                p_star=None, truth_star=None, dep_corr=0.95, weekly_steps=2, risk_cap=None, ledger_path=None):
    """
    - EV gate computed PRE-DRAW via Poisson-binomial on chosen 5 mains + 2 stars (if provided).
    ...
    pa = zscore(pa_raw, axis=1); pp = zscore(pp_raw, axis=1)
    scores = (w[0]*pa + w[1]*pp) / max(1e-6, tau); probs = softmax(scores, axis=1)
    picks_idx_main = np.argsort(probs, axis=1)[:, ::-1][:, :5]

    has_stars = (p_star is not None) and (truth_star is not None)
    if has_stars:
        ps_raw = np.asarray(p_star, dtype=np.float64)
        picks_idx_star = np.argsort(ps_raw, axis=1)[:, ::-1][:, :2]
    else:
        picks_idx_star = None

    base_table = dict(prize_table) if prize_table is not None else None
    if base_table is not None:
        base_table = validate_prize_table(base_table)

    ledger = []

    hits_main = []; cost=0.0; ret=0.0
    weekly_spend = {}
    for t in range(steps):
        week_id = t // max(1, weekly_steps)
        weekly_spend.setdefault(week_id, 0.0)

        truth_idx_main = np.where(truth[t] == 1)[0]
        hit_m = np.intersect1d(picks_idx_main[t], truth_idx_main).size
        hits_main.append(int(hit_m))

        if has_stars:
            truth_idx_star = np.where(truth_star[t] == 1)[0]
            hit_s = np.intersect1d(picks_idx_star[t], truth_idx_star).size
        else:
            hit_s = 0
        ...
        if prizes_multi is not None and len(prizes_multi) > 0:
            if len(prizes_multi) == steps:
                table_t = dict(prizes_multi[t])
            else:
                off = len(prizes_multi) - steps
                idx = t if off <= 0 else min(len(prizes_multi)-1, off + t)
                table_t = dict(prizes_multi[idx])
        elif prize_table is not None:
            table_t = dict(base_table)

        if table_t is not None:
            table_t = validate_prize_table(table_t)

        # Pre-draw EV
        p_m_list = np.clip(pa_raw[t, picks_idx_main[t]] * dep_corr, 0.0, 1.0)
        pm_m = poisson_binom_pmf(p_m_list) # poisson_binom_pmf is in the global scope of the target file
        if has_stars:
            p_s_list = np.clip(ps_raw[t, picks_idx_star[t]] * dep_corr, 0.0, 1.0)
            pm_s = poisson_binom_pmf(p_s_list) #
        else:
            pm_s = np.zeros(3); pm_s[0] = 1.0

        ev_t = 0.0
        if table_t is not None:
            for km in range(min(5, len(pm_m)-1)+1):
                for ks in range(min(2, len(pm_s)-1)+1):
                    ev_t += float(pm_m[km]) * float(pm_s[ks]) * float(table_t.get((km, ks), 0.0))

        local_mult = ev_mult
        if sig is not None and len(sig) == steps:
            s = (sig[t] - np.mean(sig)) / (np.std(sig) + 1e-12)
            local_mult *= float(1.0 + 0.05*np.tanh(s))

        placed = False; payout = 0.0
        if risk_cap is not None:
            if weekly_spend[week_id] + ticket_cost > float(risk_cap):
                placed = False
            else:
                placed = (ev_t >= local_mult * ticket_cost)
        else:
            placed = (ev_t >= local_mult * ticket_cost)

        if placed:
            weekly_spend[week_id] += ticket_cost
            cost += ticket_cost
            payout = table_t.get((hit_m, hit_s), 0.0) if table_t is not None else 0.0
            ret += payout

        ledger.append({
            "t": t, "week_id": int(week_id),
            "placed": int(placed),
            "km": int(hit_m), "ks": int(hit_s),
            "ev": float(ev_t), "payout": float(payout),
            "cost_cum": float(cost), "ret_cum": float(ret)
        })
    ...
    return np.array(hits_main, dtype=int), (cost, ret, roi), picks_idx_main, (picks_idx_star if has_stars else None)
...
def cross_entropy_search(p_agent, truth, p_pair, sig=None, prize_table=None, prizes_multi=None,
                         ...
                         p_star=None, truth_star=None, dep_corr=0.95):
    ...
    for it in range(iters):
        ...
        for w1, w2, tau in cand:
            hits, (cost, ret, roi), _, _ = eval_policy(
                ...
                p_star=p_star, truth_star=truth_star, dep_corr=dep_corr
            )
            has_prizes = (prize_table is not None) or (prizes_multi is not None)
            score = roi if (use_roi and has_prizes and not np.isnan(roi)) else hits.mean()
            scores.append(score)
...
# ---------- Stars Agent + Poisson-Binomial (added) ----------
def poisson_binomial_pmf(p_list):
    import numpy as np
    p = np.asarray(p_list, dtype=float)
    n = len(p)
    dp = np.zeros(n+1, dtype=float); dp[0] = 1.0
    for pi in p:
        dp[1:n+1] = dp[1:n+1]*(1.0 - pi) + dp[0:n]*pi
        dp[0] *= (1.0 - pi)
    return dp

def run_stars(stars_df: pd.DataFrame, cfg: LabConfig, debug=False) -> dict:
    bias_cfg = BiasConfig()
    X, y, steps, N, feat_names = build_dataset(stars_df, k=2, bias_cfg=bias_cfg, min_warmup=cfg.warmup)
    tr_steps = int((1.0 - cfg.test_frac) * steps); split_idx = tr_steps * N
    model, C, cv_loss = select_and_fit(X[:split_idx], y[:split_idx], tr_steps, N,
                                       cs_grid=(0.05,0.1,0.2,0.5,1.0,2.0), n_splits=cfg.splits, debug=debug)
    from sklearn.pipeline import Pipeline
    from sklearn.preprocessing import StandardScaler
    from sklearn.linear_model import LogisticRegression
    final = Pipeline([("scaler", StandardScaler()),
                      ("lr", LogisticRegression(penalty="l2", C=C, class_weight="balanced",
                                                solver="lbfgs", max_iter=2000))])
    final.fit(X, y)
    p_te_full = final.predict_proba(X)[:, 1]
    y_full = y
    outS = cfg.outdir / "agent_out"; outS.mkdir(parents=True, exist_ok=True)
    ...
    dump_block(truth_block, N, outS / "truth_star_test.csv")
    if debug:
        print(f"[stars] steps={steps_total} N={N} test={test_steps} C={C} cv_logloss={cv_loss:.6f}")
...
def run_all(cfg: LabConfig, mode: str, debug=False, ablate_mode="none", ablate_frac=0.0,
    ...
    p_star_path = agent_dir / "probs_star_test.csv"
    truth_star_path = agent_dir / "truth_star_test.csv"
    p_star = pd.read_csv(p_star_path, header=None).to_numpy(np.float64) if p_star_path.exists() else None
    truth_star = pd.read_csv(truth_star_path, header=None).to_numpy(np.int32) if truth_star_path.exists() else None
    shapes = [p_agent.shape[0], truth.shape[0], p_pair.shape[0]] + ([p_star.shape[0]] if p_star is not None else []) + ([truth_star.shape[0]] if truth_star is not None else [])
    T = min(shapes)
    p_agent, truth, p_pair = p_agent[:T], truth[:T], p_pair[:T]
    if p_star is not None and truth_star is not None:
        p_star, truth_star = p_star[:T], truth_star[:T]
    ...
    # Jackpot override (optional)
    if hasattr(cfg, 'jackpot_csv') and cfg.jackpot_csv and Path(cfg.jackpot_csv).exists():
        try:
            jackpots, _dfjack = load_jackpots_csv(cfg.jackpot_csv)
            if prizes_multi is not None and len(prizes_multi)>0:
                prizes_multi = inject_dynamic_jackpot(prizes_multi, jackpots, km=5, ks=2)
            else:
                print('[warn] jackpot-csv provided but no prizes_multi in use; consider --prizes-multi file.')
        except Exception as e:
            print(f'[warn] jackpot injection failed: {e}')
    ...
    pS_val, truS_val = (p_star[:val_T], truth_star[:val_T]) if p_star is not None and truth_star is not None else (None, None)
    pS_hold, truS_hold = (p_star[val_T:], truth_star[val_T:]) if p_star is not None and truth_star is not None else (None, None)
    ...
    w1,w2,tau = cross_entropy_search(
        ...
        p_star=pS_val, truth_star=truS_val, dep_corr=cfg.dep_corr
    )
    outM = cfg.outdir / "mixer_out"; outM.mkdir(parents=True, exist_ok=True)
    hits, (cost, ret, roi), picks_idx, picks_star = eval_policy(
        ...
        p_star=pS_hold, truth_star=truS_hold, dep_corr=cfg.dep_corr,
        weekly_steps=getattr(cfg,'weekly_steps',2), risk_cap=getattr(cfg,'risk_cap',None), ledger_path=(outM / 'roi_ledger.csv')
    )
    ...
    if picks_star is not None:
        pd.DataFrame((picks_star + 1)).to_csv(outM / "mixer_picks_stars.csv", index=False, header=False)
    if debug:
...
def main():
    ...
    ap.add_argument("--risk-cap", type=float, default=None, help="Max spend per week (EUR). If set, caps stakes per week window.")
    ap.add_argument("--weekly-steps", type=int, default=2, help="Number of draws per week window (default=2).")
    ap.add_argument("--jackpot-csv", type=str, default=None, help="CSV with per-draw jackpots to override prize (5,2) dynamically.")
    # Stats
