...
def parse_year(html: str, year: int, ranges: Ranges) -> List[Dict]:
    ...
    for m in patt.finditer(html):
        ...
        rec = {
            ...
            "ball_1": mains[0], "ball_2": mains[1], "ball_3": mains[2],
            "ball_4": mains[3], "ball_5": mains[4],
            "bonus": bonus,
            "draw_code": code or "",
...
def dedupe_sort(recs: List[Dict]) -> List[Dict]:
    ...
    for r in recs:
        key = (r["draw_date"],
               (int(r["ball_1"]), int(r["ball_2"]), int(r["ball_3"]), int(r["ball_4"]), int(r["ball_5"])),
...
def main() -> None:
    ...
    ap.add_argument("--years", type=int, nargs="*", default=None, help="Explicit list of years to fetch")
    args = ap.parse_args()
